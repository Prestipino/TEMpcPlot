...
def ransac_lin(data, threshDist, inlierRatio, num=2):
    ''' data: a 2xn dataset with #n data points
        ...
        threshDist: the threshold of the distances between
                            points and the fitting line
        inlierRatio: the threshold of the number of inliers express as
                     fraction 80% => 0.8
    '''
    ...
    for i in range(n_iter * 5):
        ...
        # Update the number of inliers and fitting model if better one is found
        if (inlierNum >= round(inlierRatio * number)) and (
                inlierNum > bestInNum):
            bestInNum = inlierNum
            best_fit = sample[:, :]
