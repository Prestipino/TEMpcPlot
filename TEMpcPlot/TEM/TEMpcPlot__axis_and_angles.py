...
class EwaldPeaks(object):
    ...
    def __add__(self, other):
        if len(self.inte[0]) > len(other.inte[0]):
            pos = self.pos + other.pos[1:]
            inte = self.int + other.int[1:]
        else:
            pos = self.pos[1:] + other.pos
            inte = self.int[1:] + other.int
    ...
    def refine_axes(self, axes=None, tollerance=0.1):
        ...
        res_1 = least_squares(res, axes.flatten())

        # chi square
        s_sq = (res(res_1.x)**2).sum() / (data.shape[1] * 3 - len(res_1.x))
        pcov = res_1.jac * s_sq
        error = []
    ...
    def refine_axang(self, axes=None, tollerance=0.1):
        ...
        def res(axes_ang):
            ...
            for i, pos_f_i in enumerate(pos_f[1:]):
                r = R.from_rotvec(self._rot_vect[i + 1] *33333
                                  angles[i])
        ...
        print(axes_ang[9:])
        res_1 = least_squares(res, axes_ang)
        ...
        self.set_cell(self.axes, self._axes_std)
        return (res_1.x[9:], error[9:]), res_1, pcov
